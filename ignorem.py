...
def tela_atualizar_professor():
    ...
    def opcao_troca():
        ctk.CTkLabel(frame_principal,text="ESCOLHA O QUE DESEJA MUDAR",font=("Arial",45,"bold")).grid(row=2,column=0,columnspan=3,pady=40)

        ctk.CTkButton(frame_principal,text="Mudar Nome",width=350,height=40,font=("Arial",25),command=lambda:muda_opcao("nome")).grid(row=3,column=0,padx=100,pady=15)
        ctk.CTkButton(frame_principal,text="Mudar Idade",width=350,height=40,font=("Arial",25),command=lambda:muda_opcao("idade")).grid(row=4,column=0,padx=100,pady=15)
        ctk.CTkButton(frame_principal,text="Mudar E-mail",width=350,height=40,font=("Arial",25),command=lambda:muda_opcao("e-mail")).grid(row=3,column=2,padx=100,pady=15)
        ctk.CTkButton(frame_principal,text="Adicionar Turma",width=350,height=40,font=("Arial",25),command=tela_adiciona_turma).grid(row=4,column=2,padx=100,pady=15)

    opcao_troca()

    ctk.CTkLabel(frame_principal,text="ATUALIZAR PROFESSOR",font=("Arial",45,"bold")).grid(row=0,column=0,columnspan=3,pady=40)
    ctk.CTkButton(frame_principal,text="←",width=50,height=30,command=menu_principal_admin).grid(row=0,column=0,padx=20,pady=20,sticky="nw")
    ctk.CTkButton(frame_principal,text="☀️",width=50,command=mudar_tema).grid(row=0,column=5,padx=20,pady=20,sticky="ne")

    entrar_id=ctk.CTkEntry(frame_principal,placeholder_text="ID do Professor",width=350); entrar_id.grid(row=1,column=1,pady=20)
    entrar_item=ctk.CTkEntry(frame_principal,placeholder_text="Nova alteração",width=500,height=35); entrar_item.grid(row=5,column=1,pady=35)
