...
def tela_login():
    ...
    entry_senha=ctk.CTkEntry(frame_principal,placeholder_text="Senha",show="*",width=300); entry_senha.pack(pady=10)

    def fazer_login():
        cursor.execute("SELECT * FROM usuarios WHERE usuario=%s AND senha=%s",(entry_usuario.get(),entry_senha.get()))
        if cursor.fetchone(): messagebox.showinfo("Sucesso","Login realizado!"); menu_principal()
